...
class WeighbridgeTicket(Document):
	...
	def validate_outgoing(self):
		if self.wbt_load_direction=="Outgoing":
			...
			if not self.wbt_first_weighing <= (tolerance+self.wbt_vehicle_tare_weight):
				frappe.throw(_("Tare Weight MisMatch"))

			if not self.wbt_first_weighing >= (self.wbt_vehicle_tare_weight-tolerance):
				frappe.throw(_("Tare Weight MisMatch"))
	...
	def validate_incoming(self):
		if self.wbt_load_direction=="Incoming":
			tolerance =  (1.0*self.wbt_vehicle_tare_weight/100)*10
			if not self.wbt_second_weighing <= (tolerance+self.wbt_vehicle_tare_weight):
				frappe.throw(_("Tare Weight MisMatch"))
	
			if not self.wbt_second_weighing >= (self.wbt_vehicle_tare_weight-tolerance):
				frappe.throw(_("Tare Weight MisMatch"))
