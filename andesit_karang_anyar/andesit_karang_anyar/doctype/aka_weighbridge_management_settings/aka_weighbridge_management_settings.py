...
class AKAWeighbridgeManagementSettings(Document):
	def validate(self):
...
@frappe.whitelist()
def get_aka_wb_settings(company):
	"""Return for Company - income_account, receivable_account, payable_account, tax_account and cost_center"""
	out = {
		"warehouse" : frappe.db.get_value("AKA Weighbridge Management Settings Warehouse",
		{
			"company": company
		}, "warehouse")
	}
	...
	if not out["warehouse"]:
		frappe.throw(_("Set default Warehouse in AKA Weighbridge Management Settings"))
